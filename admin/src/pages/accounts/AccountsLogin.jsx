...
// Single-column sign-in for the accounts desk, same shape as the roller's
const AccountsLogin = () => {
  ...
  const { loginAccounts, revokedNotice, setRevokedNotice } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await loginAccounts(username, password);
  ...
  return (
    <div className="flex min-h-dvh flex-col justify-center bg-slate-50 px-5 py-10">
      <div className="pointer-events-none absolute inset-x-0 top-0 h-40 bg-gradient-to-b from-amber-100/60 to-transparent" />
      ...
      <div className="relative mx-auto w-full max-w-[22rem]">
        <div className="mb-9 flex items-center gap-3">
          ...
          <div className="leading-tight">
            ...
            <p className="text-[10px] font-medium uppercase tracking-[0.18em] text-slate-500">Accounts</p>
          </div>
        </div>

        <h1 className="font-display text-2xl font-bold tracking-tight text-slate-900">Accounts sign in</h1>
        <p className="mt-1.5 text-[13px] text-slate-500">
          Track every order, queue and bill them, approve cancellations.
        </p>
        ...
        <form onSubmit={handleSubmit} className="mt-7 space-y-4">
          <div>
            <label htmlFor="accounts-username" className="mb-1.5 block">Username</label>
          ...
          <div>
            <label htmlFor="accounts-password" className="mb-1.5 block">Password</label>
            <div className="relative">
        ...
        <p className="mt-2 text-center">
          <Link to="/" className="text-[12px] font-semibold text-slate-500 underline-offset-2 hover:text-slate-700 hover:underline">
            Not an accounts manager? Change role
          </Link>
        </p>
      </div>
    </div>
